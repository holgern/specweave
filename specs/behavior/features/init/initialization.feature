...
Feature: SpecWeave project initialization
  ...
  Rule: Init creates config and directories
    ...
    Example: Init still supports explicit hidden config output
      Given an empty project directory
      When specweave init runs with an explicit ".specweave.toml" target
      Then ".specweave.toml" is created
  ...
  Rule: Init supports British spelling
    ...
    Example: Init creates behaviour layout with --spelling behaviour
      ...
      And "specs/behaviour/evidence" directory exists
      And "specs/behaviour/mappings" directory exists
